...
contract('Exchange',async([deployer,feeAccount,user1,user2])=>{
    ...
    beforeEach(async()=>{
        ...
        //Transfer some tokens to user1 for user1 for testing.
    ...
    describe('fallback',()=>{
        it('revetrs when ether is sent with smart contract',async()=>{
    ...
    describe('depositing ether',()=>{
        ...
        beforeEach(async()=>{
            amount1=ether(1);
            result = await exchange.depositEther({from:user1,value:amount1});
        })

        it('tracks ether deposit',async()=>{
            const balance = await exchange.tokens(ETHER_ADDRESS,user1);
            balance.toString().should.equal(amount1.toString());
        })
        ...
        it('emits a Deposit event', async () => {
            ...
            event.amount.toString().should.equal(amount1.toString(), 'amount is correct')
            event.balance.toString().should.equal(amount1.toString(),'balance is stored correctly')
        })
    })
    ...
    describe('withdrawing Ether',()=>{
        ...
        beforeEach(async()=>{
            amount1=ether(1);
            await exchange.depositEther({from:user1,value:amount1});
        })

        describe('success',()=>{

            beforeEach(async()=>{
                result=await exchange.withdrawEther(amount1,{from:user1});
            })
            ...
            it('emits a Withdraw event', async () => {
                ...
                event.token.should.equal(ETHER_ADDRESS, 'token is correct')
                event.user.should.equal(user1, 'user is correct')
                event.amount.toString().should.equal(amount1.toString(), 'amount withdrawn is correct')
                event.balance.toString().should.equal('0','balance is updated correctly')
            })
        })
    ...
    describe('depositing tokens',()=>{
        ...
        //need to approve token because it is authorised transfer.
        ...
        describe('success',()=>{
            ...
            it('emits a Deposit event', async () => {
                ...
                event.balance.toString().should.equal(amount.toString(),'balance is stored correctly')
            })
        })
        ...
        describe('failure',()=>{
            ...
            it('fails when no tokens are approved',async()=>{
                //we have not approved tokens for user1
    ...
    describe('Withdrawing tokens',()=>{
        ...
        // depositing first so that we can withdraw.
        beforeEach(async()=>{
            await token.approve(exchange.address,amount,{from:user1})
        ...
        describe('success',()=>{
            ...
            it('updating token balances', async () => {
                let balance;
                balance = await exchange.tokens(token.address,user1);
                balance.toString().should.equal('0');
            })
            ...
            it('emits a Withdraw event', async () => {
                ...
                event.user.should.equal(user1, 'user is correct')
                event.amount.toString().should.equal(amount.toString(), 'amount is correct')
                event.balance.toString().should.equal('0','balance is stored correctly')
            })
        })
        ...
        describe('failure',()=>{
            ...
            it('fails for insufficicent balances',async()=>{
                //available balance is tokens(100).
